function teensyOutput = program_trialBMI(vals,varargin)
%% Variable stuff
...
if ET_rewardDelivery
    ...
    soundVolume = 0;
    setSoundVolumeTeensy(soundVolume);
    giveReward2(50,1,1,0.1); % in ms. This is in the START section so that it only delivers once
end
...
if CE_experimentRunning
    ...
    plotUpdatedOutput2([CE_waitForBaseline*.9, CE_trial*.8, CE_rewardToneHold*.7,...
        ...
        , duration_plotting, frameRate, 'Rewards', 10, 20, ['Num of Rewards:  ' , num2str(NumOfRewardsAcquired)])
...
if ~isnan(counter_frameNum)
    ...
    %     NumOfRewardsAcquired = nansum(logger(:,8));
end
